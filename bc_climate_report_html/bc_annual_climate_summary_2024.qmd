...
## Read data for given month and parameter -----
  ...
  ano_dt_rast_mn_i
  #  ano_dt_rast_mn_i_prj <- project(ano_dt_rast_mn_i, "+proj=aea +lat_0=45 +lon_0=-126 +lat_1=50 +lat_2=58.5 +x_0=1000000 +y_0=0 +datum=NAD83 +units=m +no_defs" )
  # # plot( ano_dt_rast_mn_i_prj,40:44) 
  #   ano_dt_rast_mn_i <-  ano_dt_rast_mn_i_prj
  
  # Crop for BC
    ano_dt_bc_mon_i_rast <-
      terra::crop(ano_dt_rast_mn_i, bc_shp, mask = T)
    ano_dt_bc_mon_i_rast
   
    ano_dt_bc_mon_i_rast_cur_yr <- subset(ano_dt_bc_mon_i_rast,   which(names(ano_dt_bc_mon_i_rast) %in%   mon_i_date))
    # plot(ano_dt_bc_mon_i_rast_cur_yr)
...
# for final plot
bc_mon_ano_cur_yr_dt_prj <- project(bc_mon_ano_cur_yr_dt,'+proj=aea +lat_0=45 +lon_0=-126 +lat_1=50 +lat_2=58.5 +x_0=1000000 +y_0=0 +datum=NAD83 +units=m +no_defs' )
bc_mon_ano_cur_yr_dt_prj <- mask(bc_mon_ano_cur_yr_dt_prj, bc_shp_prj)

bc_mon_ano_plt_cur_yr <- ggplot()+
  geom_spatraster(data = bc_mon_ano_cur_yr_dt_prj)+
...
ano_dt_shp_rast <- crop(ano_dt_rast,bc_shp, mask= T)
...
ano_dt_rast_cur_yr_prj <- project(ano_dt_rast_cur_yr,'+proj=aea +lat_0=45 +lon_0=-126 +lat_1=50 +lat_2=58.5 +x_0=1000000 +y_0=0 +datum=NAD83 +units=m +no_defs' )
ano_dt_rast_cur_yr_prj <- mask(ano_dt_rast_cur_yr_prj,bc_shp_prj)

bc_ann_ano_plt_cur_yr <- ggplot()+
  geom_spatraster(data = ano_dt_rast_cur_yr_prj)+
  scale_fill_gradientn(
    name = paste0(parr, " anomaly ", unt),
    colours = cpt(pal = "ncl_BlWhRe",
                  n = 100,
...
# Spatial Trend:: annual
 ano_trn_mag_shp <- crop(ano_trn_mag, bc_shp, mask = T) 
...
This report summarizes British Columbia (BC)'s climate conditions in 2024 based on three key variables: average temperature, total precipitation, and soil moisture drived from <a href='https://cds.climate.copernicus.eu/cdsapp#!/dataset/reanalysis-era5-land?tab=overview'>ERA5-Land hourly dataset</a>.

# BC temperature in 2024

In 2024, the average annual temperature in BC increased by 0.96°C, ranking it as the 8^th^ warmest year on record since 1950 (@fig-bc_ann_temp_ano_ts). Globally, 2024 was the warmest year ever recorded[^1]; however, in BC, the warmest year remains 2023, which saw an average temperature increase of 2.15°C. Notably, seven of the 10 warmest years in BC have occurred since 2010 (@tbl-bc_ann_temp_rank) .

[^1]: Tollefson, J. Earth shattered heat records in 2023 and 2024: is global warming speeding up? Nature 637, 523–524 (2025).
...
Between 1950 (1980) and 2024, BC's average temperature rose significantly, increasing by 2.21°C (1.07°C) based on the ERA5-Land data.
...
Overall, precipitation amount in BC was slightly lower the normal (-2.73% anomaly) in 2024 relative to the 1981-2010 climatological average. This marks a continuation of the declining total precipitation observed since 2022 (@fig-bc_ann_prcp_ano_ts). However, there was considerable spatial variability across the province. Northeastern BC experienced a reduction in precipitation exceeding 30%, while the Central Interior recorded increases of up to 25% (@fig-bc_ann_ano_prcp_dist_plt).
...
#| fig-cap: !expr "paste('Monthly total precipitation anomalies (% of normal) for BC in 2024. The greenish areas represent wetter-than-normal precipitation areas, while the brownish areas indicate drier-than-normal precipitation regions.Data credit: *ERA5land/C3S/ECMWF*')"
...
# Soil moisture and drought in 2024

Volumetric soil moisture is an important indicator of water availability for plant uptake, groundwater recharge, drought, and other hydrological processes. While the average annual soil moisture in 2024 was close to normal (-0.58%) (@fig-bc_ann_soil_ano_ts),there were significant spatial and monthly variations across BC (@fig-bc_ann_ano_soil_dist_plt). In Northeast BC and the Peace Region, soil moisture remained below normal throughout the year, with levels over 30% lower than normal during the summer and fall months (@fig-bc_mon_soil_dist_plt). July, August, and September experienced, on average, 7% ,6%, and 4% lower than normal soil moisture content respectively making drier months even more dry under the surface (@fig-bc_mon_soil_dist_plt). This further restricted water availability for vegetation and crops during the growing season.

The drought conditions from 2023 continued into 2024, with lower-than-normal precipitation (@fig-bc_mon_prcp_dist_plt) and below normal snowpacks for most of the province during the winter months ( see <a href= 'https://www2.gov.bc.ca/gov/content/environment/air-land-water/water/drought-flooding-dikes-dams/river-forecast-centre'> River Forecast Center's </a> Snow Survey and Water Supply Bulletins <a href= 'https://www2.gov.bc.ca/assets/gov/environment/air-land-water/water/river-forecast/2024.pdf'> report of 2024 </a> ).

```{r soil mositure timeseries anomaly plt}
#| include: true
#| output: true
#| cache: false
#| fig-align: left
...
#| fig-cap: !expr "paste('Annual volumetric soil moisture ( 0-1 m) anomalies ( % of normal) for BC 1950 - 2024. The greenish bars represent wetter-than-normal years, while the brownish bars indicate drier-than-normal years. Anomalies are calculated relative to the 1981–2010 climatological baseline. The magenta line represents the moving regression. Data credit: *ERA5land/C3S/ECMWF*')"


parrs_ann_summary_results_lst[[3]]$soil_moisture_ann_ano_lin_plt_bc_2024+
  theme(
  plot.title = element_text(size = rel(2)),   
  axis.title.x = element_text(size = rel(2)),
  axis.title.y = element_text(size = rel(1.5)),
...
```{r soil mositure ann ano dist plt}
#| include: true
#| output: true
#| cache: false
#| fig-align: left
...
#| fig-cap: "BC's average volumetric soil moisture anomaly for 2024 relative to the 1981–2010 reference period. The greenish areas represent wetter-than-normal mositure areas, while the brownish areas indicate drier-than-normal mositure areas for 2024. Data credit: *ERA5land/C3S/ECMWF*"


parrs_ann_summary_results_lst[[3]]$soil_moisture_ann_ano_dist_plt_bc_2024+
  theme(
  plot.title = element_text(size = rel(1.0)))

```

```{r soil mositure mon ano dist plt}
#| include: true
#| output: true
#| cache: false
#| fig-align: left
...
#| fig-cap: !expr "paste('Monthly volumnetric soil mositure (0-1 m depth) anomalies (% of normal) for BC in 2024. The greenish areas represent wetter-than-normal soil mositure areas, while the brownish areas indicate drier-than-normal soil mositure conditions.Data credit: *ERA5land/C3S/ECMWF*')"

 parrs_ann_summary_results_lst[[3]]$soil_moisture_mon_ano_dist_plt_bc_2024+
  theme(
  plot.title = element_text(size = rel(2)))

```

# Factors influcing warming and precipitation of BC in 2024

Globally the increase of the greenhouse gases concentration in the atmosphere due to fossil fuel combustion by humans is driving long-term warming. However, natural factors such as the El Niño phase of the El Niño-Southern Oscillation (ENSO), a naturally occurring periodic warming and cooling of ocean waters in the eastern and central tropical Pacific, as well as large volcanic eruptions and the solar cycle play a role in influencing short-term global and regional temperatures. The year 2024 continued with the warming phase of ENSO (El Niño) since 2023 until spring of 2024. During El Niño BC typically experiences warmer temperature therefore less precipitation as snowfall. Moreover, some reports suggest the role of Hunga Tonga volcanic eruption and reduction in marine fuel pollution to atmosphere also contributed to global temperature increase especially for past few years [^2].

[^2]: [Berkeleyearth Global Temperature Report 2024](https://berkeleyearth.org/global-temperature-report-for-2024/)

::: {style="font-size: 70%;"}
**Data issues and Disclaimer:** This report has been prepared using <a href='https://www.ecmwf.int/en/era5-land'>ERA5-Land</a> data from the European Centre for Medium-Range Weather Forecasts (ECMWF), as available at the time of preparation. Please note that the original data may be subject to updates or revisions. Any modifications to the original data may result in adjustments to the findings presented in this report. There may be known or unknown issues with the data. For example, the ERA5-Land summertime time series for the year 1979 is affected by inhomogeneity. Users should acknowledge these limitations when using this report.
:::

```{r copy report to www}
#| include: False
 file.copy(from = 'bc_annual_climate_summary_2024.html',
           to = "../www/", overwrite = T)
```
