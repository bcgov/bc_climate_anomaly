...
r_lines <- gsub('^update_month\\s*<-\\s*\\".*\\"', paste0('update_month <- "', update_month, '"'), r_lines)
r_lines <- gsub('^update_year\\s*<-\\s*\\".*\\"', paste0('update_year <- "', update_year, '"'), r_lines)
...
qmd_lines <- gsub('^update_month\\s*<-\\s*\\".*\\"', paste0('update_month <- "', update_month, '"'), qmd_lines)
qmd_lines <- gsub('^update_year\\s*<-\\s*\\".*\\"', paste0('update_year <- "', update_year, '"'), qmd_lines)

# Write modified QMD to file
writeLines(qmd_lines, updated_qmd_file)

# Render the updated QMD file
html_file_name <- paste0(update_month, "_", update_year, "_bc_mon_sea_ann_climate_summary.html")
...
unlink("2_bc_mon_sea_ann_clmt_summary_quarto_report_UPDATED_cache", recursive=TRUE)
