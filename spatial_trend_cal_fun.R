...
spatial_trend_cal_fun <- function(parr, ano_dt_fl,clm_dt_fl, monn) {
  ...
  for (i in 1:length(mons_unq)){
   ...
   trend_stack <- tryCatch({
     ...
     message(glue::glue("Trend calculation failed for '{parr}', month '{ mons_unq_i}': {e$message}"))
 ...
 names(trend_stack) <- c("trend_mag", "pval")
   ...
   output_file <- file.path(
     sptl_trn_dt_pth,
     glue::glue("{parr}_spatial_trend_mon_{mons_unq_i}_wna_{start_year}_{end_year}.nc")
   )
   terra::writeCDF(
     trend_stack,
  ...
  yr_df %>%
   ...
   ano_dt_rast80 <- subset(ano_dt_rast, which(names(ano_dt_rast) %in% yr_df80$yr))
   # plot(ano_dt_rast80)
   ...
   output_file <- file.path(
     sptl_trn_dt_pth,
     glue::glue("{parr}_spatial_trend_mon_{mons_unq_i}_wna_{start_year80}_{end_year80}.nc")
   )
   terra::writeCDF(
     trend_stack80,
     filename = output_file,
...
spatial_trend_cal_fun(parr =  par_j,
                      ano_dt_fl = ano_dt_fl,
                      clm_dt_fl = clm_dt_fl,
                      monn = monns)
}
